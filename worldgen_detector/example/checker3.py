...
def find_tree_in_place(id):
    ...
    for t in treeList:
        if t in id:
            result.append(f"{t_mod_id}:{t}")
            global txt_info
            txt_info += f"{id} found {jt.dictToJsonNoOpen( get_config(get_place(id)['feature']))}\n"
            print( f"\"{get_config(get_place(id)['feature'])['type']}\",")
    return result
